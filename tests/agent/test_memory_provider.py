...
class TestPluginMemoryDiscovery:
    ...
    def test_load_provider_by_name(self):
        ...
        # Honcho is config-gated in clean environments, so mock the config
        # layer rather than depending on local setup or CI secrets.
        with patch("plugins.memory.honcho.client.HonchoClientConfig.from_global_config") as mock_cfg:
            mock_cfg.return_value = MagicMock(enabled=True, api_key="test-key", base_url=None)
